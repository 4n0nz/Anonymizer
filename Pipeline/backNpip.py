...
def compose_screen_pip(bg, screen, pip_vid, meta, out_tmp):
    ...
    # Coordonnées metadata → espace display
    masked_w = min(scr_w, C.MAX_WIDTH)
    masked_h = int(scr_h * masked_w / scr_w)

    # SAFE ZONE = taille pour couvrir le personnage (16:9)
    if "safe_x" in meta:
        # Ancrage = coin sup-gauche INITIAL du crop sauvegardé lors de l'extraction
        p_x = s_x + int(meta["x"] * s_w / masked_w)
        p_y = s_y + int(meta["y"] * s_h / masked_h)

        # Taille = safe zone × zoom, ratio 16:9
        zoom = getattr(C, "PIP_ZOOM_FACTOR", 1.0)
        p_w = even(int(meta["safe_w"] * s_w / masked_w * zoom))
        p_h = even(int(p_w * 9 / 16))

        print(f"  [SAFE] PIP ancré crop ({p_x},{p_y}) zoom={zoom}x → {p_w}x{p_h}")
    else:
        # Fallback ancienne metadata sans safe zone
    ...
    filter_complex = (
        ...
        f"[tmp][pip]overlay={p_x}:{p_y}:eof_action=pass[outv]"
    )
