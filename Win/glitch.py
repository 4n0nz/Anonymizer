...
def mask_present(frame):
    # Detection du marqueur vert laisse par face_mask.py
...
def apply_vhs_crt_glitch(frame, rng):
    ...
    # --- ECRASEMENT R/B (signature CRT verte) ---
...
def process_video(video_path, temp_out):
    ...
    ret, frame = cap.read()
    if not ret:
        raise RuntimeError("Impossible de lire la premiere frame")
...
def main():
    ...
    if not videos:
        raise RuntimeError("Aucune video a traiter")
