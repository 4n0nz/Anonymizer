...
# -*- coding: utf-8 -*-
"""
Downloader.py — Video Downloader (OpenClassrooms + YouTube)
Télécharge vidéos + sous-titres pour usage offline.
...
Usage :
    # OpenClassrooms
    python Downloader.py <url_du_cours>
    python Downloader.py <url_du_cours> --list
    python Downloader.py <url_du_cours> --quality 720

    # YouTube (vidéo, playlist, channel)
    python Downloader.py https://www.youtube.com/watch?v=...
    python Downloader.py https://www.youtube.com/playlist?list=...
    python Downloader.py https://youtu.be/... --quality 1080
"""
...
# ocr_credentials.py est dans Pipeline/ (template : ocr_credentials.example.py)
...
def simplify_filename(filename, max_words=4):
    """Simplifie un nom : retire les caractères spéciaux et limite à N mots.

    Ex: "Ma_Super_Video_Avec_(Bonus)_Tres_Longue_abc123.mp4"
        → "Ma_Super_Video_Avec.mp4"
    """
    name, ext = os.path.splitext(filename)

    # Retire les caractères spéciaux (parenthèses, crochets, virgules, etc.)
    name = re.sub(r'[()\[\]{}<>:"/\\|?*,;!@#$%^&+=~`\'’“”]', '', name)

    # Split par underscore, espace ou tiret
    words = re.split(r'[_\s\-]+', name)

    # Filtre les chunks vides et les IDs YouTube (hex de 8+ chars)
    clean = [
        w for w in words
        if w and not re.fullmatch(r'[0-9a-fA-F]{8,}', w)
        and not re.fullmatch(r'[0-9_]{3,}', w)  # pas que des chiffres
    ]

    if not clean:
        return None  # nom inutilisable

    selected = clean[:max_words]
    return "_".join(selected) + ext.lower()


def simplify_downloads(directory, before_set=None):
    """Renomme les fichiers téléchargés selon simplify_filename().
    Si before_set fourni → ne renomme que les fichiers nouveaux."""
    directory = Path(directory)
    if not directory.is_dir():
        return

    current = set(os.listdir(directory))
    targets = current - before_set if before_set else current

    for f in targets:
        # On simplifie uniquement les vidéos (les .vtt/.jpg suivent automatiquement)
        if not f.lower().endswith((".mp4", ".mov", ".avi", ".mkv", ".webm")):
            continue

        new_name = simplify_filename(f)
        if not new_name or new_name == f:
            continue

        old_path = directory / f
        new_path = directory / new_name

        # Évite la collision : si le nouveau nom existe déjà, on ajoute un suffixe
        i = 2
        while new_path.exists() and old_path != new_path:
            stem, ext = os.path.splitext(new_name)
            new_path = directory / f"{stem}_{i}{ext}"
            i += 1

        try:
            os.rename(old_path, new_path)
            print(f"  [RENAME] {f}  →  {new_path.name}")

            # Renomme aussi les fichiers compagnons (subs, thumbnails)
            old_stem = os.path.splitext(f)[0]
            new_stem = os.path.splitext(new_path.name)[0]
            for companion in list(os.listdir(directory)):
                if companion.startswith(old_stem) and companion != new_path.name:
                    new_comp = companion.replace(old_stem, new_stem, 1)
                    try:
                        os.rename(directory / companion, directory / new_comp)
                    except OSError:
                        pass
        except OSError as e:
            print(f"  [WARN] Impossible de renommer {f} : {e}")


# ======================
# DETECTION DU TYPE DE SOURCE
# ======================

def is_youtube_url(url):
    """Détecte si l'URL est une vidéo ou playlist YouTube."""
...
def is_openclassrooms_url(url):
    return "openclassrooms.com" in url.lower()
...
def download_youtube(url, output_dir, quality="best"):
    """Télécharge une vidéo ou playlist YouTube via yt-dlp.
    Supporte : vidéo simple, playlist, channel."""

    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    # Format selon la qualité demandée
    if quality == "best":
    ...
    # Template de nommage : titre_id.ext (évite les collisions)
    ...
    cmd = [
        ...
        "--restrict-filenames",   # pas d'espaces ni caractères spéciaux dans le nom
        "-o", output_tmpl,
        "--no-overwrites",
        "--ignore-errors",         # continue si une vidéo de la playlist foire
        url,
    ]

    print(f"[YT] Téléchargement → {output_dir.resolve()}")
    print(f"[YT] Format : {fmt}")
    print()

    # Snapshot avant pour identifier les fichiers nouveaux
    before = set(os.listdir(output_dir)) if output_dir.exists() else set()
    ...
    # Post-traitement : simplification des noms (max 4 mots, pas de caractères spéciaux)
    if success:
        print()
        print("[YT] Simplification des noms de fichiers...")
        simplify_downloads(output_dir, before_set=before)
...
def _yt_dlp_common(cookies_path, quality, referer=OCR_BASE + "/"):
    ...
    return [
        ...
        "--no-overwrites",
        "--progress",
        "--restrict-filenames",  # pas d'espaces ni caractères spéciaux
...
def download_videos(course_url, session, cookies_path, output_dir, quality="best",
    ...
    # Plus de sous-dossier : tout va direct dans output_dir (= input/)
    # On préfixe juste le nom du cours pour distinguer
    safe_title = re.sub(r'[<>:"/\\|?*\s]+', "_", course_title).strip("_") or "cours"
    course_dir = output_dir   # alias pour la suite
    ...
    if not activities:
        ...
        out_tmpl = str(course_dir / f"{safe_title}_%(playlist_index|00)02d.%(ext)s")
        cmd = (
    ...
    for act in activities:
        ...
        # Nom : <cours>_<index>_<titre>, sans espaces ni caractères spéciaux
        safe = re.sub(r'[<>:"/\\|?*\s]+', "_", title)[:60].strip("_") or f"act{idx}"
        out_tmpl = str(course_dir / f"{safe_title}_{idx:02d}_{safe}.%(ext)s")
    ...
    # Post-traitement : simplification des noms
    if ok_count > 0:
        print()
        print("[*] Simplification des noms de fichiers...")
        simplify_downloads(course_dir)
...
def main():
    ...
    parser.add_argument("url",              help="URL OpenClassrooms ou YouTube (vidéo/playlist/channel)")
    ...
    print()
    print("  ╔═══════════════════════════════════════════╗")
    print("  ║   Video Downloader (OpenClassrooms / YT)  ║")
    print("  ╚═══════════════════════════════════════════╝")
    print()
    ...
    # ── Dispatch selon le type d'URL ──
    if is_youtube_url(args.url):
        print(f"[SOURCE] YouTube détecté")
        success = download_youtube(args.url, output_dir, args.quality)
        if success:
            print(f"\n  ✓ Terminé ! Fichiers dans : {output_dir.resolve()}\n")
            return
        else:
            print("\n[ERREUR] Téléchargement YouTube échoué.")
            sys.exit(1)

    # Sinon : OpenClassrooms (logique existante)
    if not is_openclassrooms_url(args.url):
        print(f"[WARN] URL ni YouTube ni OpenClassrooms — tentative en mode OC")
